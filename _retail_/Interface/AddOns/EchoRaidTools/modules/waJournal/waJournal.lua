...
local function createContentFrame()
	...
	EncounterJournal.encounter.info.waJournal = contentFrame;
end
...
local function createJournalTab()
	...
	EncounterJournal.encounter.info.waJournalTab = tabButton;
	...
	EJ_Tabs[5] = {frame="waJournal", button="waJournalTab"};
	
	local function localSetTab(tabType)
		...
		info.difficulty:SetShown(false);
	end

	tabButton:SetScript("OnClick",function(self,button)
		localSetTab(ourTabIndex);
...
function journal:ADDON_LOADED(addonName)
	if(addonName == "Blizzard_EncounterJournal")then
		if (not C_AddOns.IsAddOnLoaded("!!!EchoInternal")) then
