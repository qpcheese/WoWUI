...
local GenerateButtonList = function()
    ...
    CreateButton(
            ...
            function(self)
                ...
                --AdventureGuideUtil.OpenJournalLink(0,1207,16)
                --EncounterJournal_OpenJournal(4, 1207)
                ShowUIPanel(EncounterJournal)
                --EncounterJournalRaidTab:Click()
                --local id = EncounterJournal.raidsTab:GetID()
                ...
                --[[EncounterJournal_DisplayInstance(1207, true) -- Amir]]
    ...
    for index, button in ipairs(_EchoRaidTools.moduleButtons or {}) do
        if index == 1 then
            ...
            button:SetPoint("TOPLEFT", _EchoRaidTools.panels.main.MainMenu.ScrollFrame.Content, "TOPLEFT", 5, -27)
...
function _EchoRaidTools:SetStatus(text, duration) -- optional duration
    if _EchoRaidTools.panels and _EchoRaidTools.panels.main then
        local returnText = _EchoRaidTools.panels.main.StatusFrame.status:GetText()
        if timer then
            timer:Cancel()
            timer = nil
            returnText = ""
        ...
        if duration then
            timer = C_Timer.NewTimer(duration,
                function()
                    _EchoRaidTools.panels.main.StatusFrame.status:SetText(returnText)
                end
            )
        end
        _EchoRaidTools.panels.main.StatusFrame.status:SetText(text)
    end
end
