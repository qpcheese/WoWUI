...
local module = MER:GetModule("MER_ContextMenu")
	...
	GUILD_INVITE = {
		...
		func = function(contextData)
			...
			elseif contextData.name then
				...
				module:Log("debug", "Cannot get the name.")
			end
		end,
	...
	ARMORY = {
		...
		func = function(frame)
			...
			if name and server then
				local link = module:GetArmoryBaseURL() .. server .. "/" .. name
				E:StaticPopup_Show("ELVUI_EDITBOX", nil, nil, link)
			else
				module:Log("debug", "Cannot get the armory link.")
			end
		end,
		hidden = function(contextData)
			if contextData.unit and contextData.unit == "target" then
...
module.TypeToFeatureMap = {}
for feature, featureConfig in pairs(module.Features) do
	for supportType in pairs(featureConfig.supportTypes) do
		if not module.TypeToFeatureMap[supportType] then
			module.TypeToFeatureMap[supportType] = {}
		end
		tinsert(module.TypeToFeatureMap[supportType], feature)
	end
end
...
function module:ModifyMenu(_, rootDescription, contextData)
...
function module:Initialize()
	self.db = E.db.mui.misc.contextMenu
	if not self.db.enable or self.initialized then
		return
	end

	local sectionText = MER.PlainTitle
	if not MER.ChineseLocale then
		sectionText = sectionText .. " "
	end
	self.sectionName = F.cOption(sectionText .. L["Menu"], "gradient")
